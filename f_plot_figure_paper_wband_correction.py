...
def f_calculateExactRadarTime(millisec, chirpIntegrations, datetimeRadar):
    ...
    timeRadar           =  datetimeRadar.astype(np.int64)/10**9
...
w_ship_shifted = xr.open_dataset(pathOutData+date+'_'+hour+'_w_ship.nc')
print('* read radar data for the selected hour')

radarData          = xr.open_dataset(radarFileName, decode_times=False)
mdv                = radarData['vm'].values
timeRadar          = radarData['time'].values
unitsRadar         = 'seconds since 2001-01-01 00:00:00'
datetimeRadar      = pd.to_datetime(timeRadar, unit='s', origin=pd.Timestamp('2001-01-01 00:00:00'))
...
mesh = axs[2].pcolormesh(datetimeRadar, rangeRadar, ma.masked_invalid(mdv_roll3.values).T, vmin=mincm, vmax=maxcm, cmap='seismic', rasterized=True)
