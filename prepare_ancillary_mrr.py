...
Eurec4aHours     = pd.date_range(datetime(2020,2,16),datetime(2020,2,19),freq='h')
NHoursEurec4a    = len(Eurec4aHours)
...
for indHour in range(NHoursEurec4a):


    yy              = str(Eurec4aHours[indHour].year)
    mm              = '0'+str(Eurec4aHours[indHour].month)
    dd              = str(Eurec4aHours[indHour].day)
    hh              = str(Eurec4aHours[indHour].hour)
    
    if len(dd) == 1:
        dd = '0'+dd
    if len(hh) == 1:
        hh = '0'+hh
    date_hour       = yy+mm+dd+'_'+hh
    ...
    # read height from one radar file to establish vertical regridding of the data
    MRRData = xr.open_dataset('/Volumes/Extreme SSD/ship_motion_correction_merian/mrr/first_step_claudia_processed/02022020_23_preprocessedClau_4Albert.nc')
    height = MRRData.height.values


    # check if file for w-ship correction has already been processed or not
    if os.path.isfile(pathNcDataAnc+'ancillary_mrr/'+dd+mm+yy+'_'+hh+'_w_ship_mrr.nc'):

        print('ship motions already calculated')
        w_ship_shifted = xr.open_dataset(pathNcDataAnc+'ancillary_mrr/'+dd+mm+yy+'_'+hh+'_w_ship_mrr.nc')
        ...
        timeStart    = datetime(int(yy),int(mm), int(dd),int(hh),0,0)
        if hh != '23':
            timeEnd      = datetime(int(yy),int(mm), int(dd),int(hh)+1,0,0)
        else:
            timeEnd      = datetime(int(yy),int(mm), int(dd),int(hh),59,59)
        timeArr         = pd.to_datetime(np.arange(timeStart, timeEnd, timedelta(seconds=1), dtype='datetime64'))

        print('processing '+date+', hour :'+hh)
        print('timestart' , timeStart)
        print('timeEnd', timeEnd)
        print('**************************************')
        print('* reading correction terms for the day/hour :'+yy+'-'+mm+'-'+dd+', hour: '+hh)
        ...
        w_ship_shifted.to_netcdf(pathNcDataAnc+'ancillary_mrr/'+date+'_'+hh+'_w_ship_mrr.nc')
        print('file saved for : '+date+'_'+hh)
        print("************************************************")