...
# -*- coding: utf-8 -*-
"""
Created on Wed May 19 17:31:47 2021
...
def f_fftAnalysisForCheck(w_radar_orig, timeRadar, wShip, timeShip, W_corr, W_corr_no_shift, PathFigHour, date, hour):
    ...
    ax.set_title('a)', fontsize=fontSizeTitle, loc='left')
    ...
    axtt.legend(frameon=False, fontsize=16)
    axtt2 = ax.twiny()
...
def f_fftAnalysisForCheck_v2(w_radar_orig, timeRadar, wShip, timeShip, W_corr, W_corr_no_shift, v_trasl, v_rot, PathFigHour, date, hour):
    '''
    author: Claudia Acquistapace
    date  : 20/05/2021
    goal  : given the velocity time series for the various variables, calculates 
    ...
    CsTrasl   = CubicSpline(pd.to_datetime(timeShip),v_trasl)
    w_trasl   = CsTrasl(pd.to_datetime(timeRadar))
    
    CS_rot    = CubicSpline(pd.to_datetime(timeShip),v_rot)
    w_rot     = CS_rot(pd.to_datetime(timeRadar))
    ...
    nans, x               = nan_helper(w_trasl[:])
    w_trasl[nans]          = np.interp(x(nans), x(~nans), w_trasl[~nans])  
    
    nans, x               = nan_helper(w_rot[:])
    w_rot[nans]          = np.interp(x(nans), x(~nans), w_rot[~nans])  
    ...
    pow_radarCorr_NS, freq_radarCorr_NS = f_calcFftSpectra(W_corr_no_shift, pd.to_datetime(timeRadar))  
    pow_wShip, freq_Ship                = f_calcFftSpectra(W_ship, pd.to_datetime(timeRadar))  
    pow_wtrasl, freq_trasl              = f_calcFftSpectra(w_trasl, pd.to_datetime(timeRadar))  
    pow_wrot, freq_rot                  = f_calcFftSpectra(w_rot, pd.to_datetime(timeRadar))  
    pow_radarOrig, freq_radarOrig       = f_calcFftSpectra(w_radar_orig, pd.to_datetime(timeRadar))                  
   ...
   # positions of the ticks for the period that appears as second axis
    ...
    ############# new figure ########################
    fig, axs = plt.subplots(3, 1, figsize=(14,14), constrained_layout=True) #sharex=True, 
    ...
    axs[0].loglog(freq_Ship, pow_wShip, label='ship', color='black', alpha=0.5, marker=".")
    axs[0].loglog(freq_trasl, pow_wtrasl, label='v_trasl (heave)', color='purple')
    axs[0].loglog(freq_rot, pow_wrot, label='v_rot', color='cyan')

    axs[0].loglog(freq_radarOrig, pow_radarOrig, label='w radar original', color='orange', linestyle=":")
    axs[0].legend(frameon=False, fontsize=16)
    axs[0].set_ylabel('FFT spectra [s$^{-1}$]', fontsize=fontSizeX)
    axs[0].axvline(x=0.155, color='black', linestyle=':')
    axs[0].axvline(x=0.055, color='black', linestyle=':')
    ...
    axs[1].loglog(freq_radarOrig, pow_radarOrig, label='w radar original', color='black')
    axs[1].loglog(freq_radarCorr, pow_radarCorr, label='w radar corrected with time shift', color='pink') 
    ...
    axs[1].axvline(x=0.055, color='black', linestyle=':')
    ...
    axs[2].loglog(freq_radarOrig, pow_radarOrig, label='w radar original', color='black')
    axs[2].loglog(freq_radarCorr_NS, pow_radarCorr_NS, label='w radar corrected without time shift', color='green')    
    axs[2].set_ylabel('FFT spectra [s$^{-1}$]', fontsize=fontSizeX)
    axs[2].set_xlabel('frequency [s]', fontsize=fontSizeX)
    axs[2].legend(frameon=False, fontsize=16)
    ...
    fig.savefig(PathFigHour+date+'_'+hour+'_fft_analysis.png', format='png')
    fig.savefig(PathFigHour+date+'_'+hour+'_fft_analysis.pdf', format='pdf')
    ############# end new figure ########################
...
for indDay in range(NdaysEurec4a):
    ...
    for indHour in range(len(radarFileList)):
        ...
        if make_fft_plot == 0:
            ...
            pathFigFFT = '/Volumes/Extreme SSD/ship_motion_correction_merian/plots/paperPlots/'
            ...
            if os.path.exists(radarFileList[indHour]):
                radarData          = xr.open_dataset(radarFileList[indHour], decode_times=False)
                datetimeRadar      = nc4.num2date(radarData.time.values, units='seconds since 2001-01-01 00:00:00', only_use_cftime_datetimes=False, only_use_python_datetimes=True) 
                radarData['time']  = datetimeRadar
                ...
                v_trasl           = w_ship_shifted['V_trasl'].values[2,:]
                v_rot             = w_ship_shifted['V_rot'].values
                ...
                for indHeight in range(len(rangeRadar)):
                    ...
                    if indHeight == 192:  
                        ...
                        f_fftAnalysisForCheck_v2(mdv[:,indHeight], \
                                                  timeChirp1, \
