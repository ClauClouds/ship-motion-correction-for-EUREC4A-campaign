...
for indDay, dayEu in enumerate(Eurec4aDays):
    ...
    Path(pathFig+'/'+dayEu+'/').mkdir(parents=True, exist_ok=True)
    ...
    for iHour, filename in enumerate(radarFileList):
        ...
        for ivar in range(len(varStringArr)):
            ...
            fig.tight_layout()
            plt.savefig(pathFig+'/'+dayEu+'/'+yy+mm+dd+'_'+hh+'_'+radar_name+'_'+varString+'_quicklooks.png', format='png', bbox_inches='tight')
        ...
        # -----------------------------------------------------------------------
        plt.savefig(pathFig+'/'+dayEu+'/'+yy+mm+dd+'_'+hh+'_'+radar_name+'_metstation.png')
