...
time_end = datetime(2020,2,12,18,0,0)
...
colorsUpper_vd = ["#fd2c3b", "#59413f", "#fdc7cc", "#8f323c", "#e66c4c", "#ae8788"] #MDV
#cmap_vd, ticks_vd, norm_vd, bounds_vd =  f_defineDoubleColorPalette(colorsLower_vd, colorsUpper_vd, mincm_vd, maxcm_vd, step_vd, thrs_vd)
cmap_vd, ticks_vd, norm_vd, bounds_vd =f_defineSingleColorPalette(colorsLower_vd, mincm_vd, maxcm_vd, step_vd)
...
matplotlib.rc('ytick', labelsize=26)  # sets dimension of ticks in the plots
# build colorbar
mesh = axs[0].pcolormesh(time_mrr, range_mrr,  Ze.T, vmin=mincm_ze, vmax=maxcm_ze, cmap=cmap_ze, rasterized=True)
...
mesh = axs[1].pcolormesh(time_mrr, range_mrr, -fall_speed.T, vmin=-8., vmax=0., cmap=cmap_vd, rasterized=True)
...
for ax, l in zip(axs.flatten(), [ '(a) Reflectivity - MRR-PRO',  '(b) Rain Fall speed - MRR-PRO ', '(c) Rainfall rate - MRR-PRO']):
    ax.text(-0.05, 1.05, l,  fontweight='black', fontsize=20, transform=ax.transAxes)
...
plt.scatter(ze_profile, -sk_profile, color= )
...
cmap_sk, ticks_sk, norm_sk, bounds_sk =  f_defineDoubleColorPalette(colorsLower_sk, colorsUpper_sk, mincm_sk, maxcm_sk, step_sk, thrs_sk)
cbarstr = 'Sk []'
spectra_sliced = spectra_file.sel(time=slice(datetime(2020,1,28,9,45,0), datetime(2020,1,28,11,15,0)))

skewness_full = spectra_sliced.skew.values
time_full = spectra_sliced.time.values
fontSizeX = 26

fig, axs = plt.subplots(2, 1, figsize=(20,20), sharex=True, constrained_layout=True)
# setting dates formatter 
[a.xaxis.set_major_formatter(mdates.DateFormatter('%H:%M')) for a in axs[:].flatten()]
matplotlib.rc('xtick', labelsize=26)  # sets dimension of ticks in the plots
matplotlib.rc('ytick', labelsize=26)  # sets dimension of ticks in the plots
...
mesh = axs[0].pcolormesh(timeLocal, rangeRadar, -Sk.T, vmin=mincm_sk, vmax=maxcm_sk, cmap=cmap_sk, rasterized=True)
...
axs[0].set_xlabel('Time UTC [hh:mm]', fontsize=fontSizeX)
axs[0].plot(lcl_time, lcl, color='black', linestyle='dotted', linewidth=3)
axs[0].axvline(x=time_profile, color='black', linewidth=4, linestyle=':')

mesh = axs[1].pcolormesh(time_full, rangeRadar,  skewness_full.T, vmax=maxcm_sk, vmin=mincm_sk, cmap=cmap_sk, rasterized=True)
...
axs[1].set_xlim(startTime, endTime)
cbar = fig.colorbar(mesh, ax=axs[1], location='right', aspect=10, use_gridspec=grid)
cbar.set_label(label='sk from complete file []',  size=26)
axs[1].set_ylabel('Height [m]', fontsize=fontSizeX)
