...
grid            = True
fig, axs = plt.subplots(4, 1, figsize=(14,14), sharex=True, constrained_layout=True)
[a.xaxis.set_major_formatter(mdates.DateFormatter('%d %b')) for a in axs.flatten()]
...
axs[3].plot(datetimeShip, yawShipArr,  color='orchid',  rasterized=True)
axs[3].axhline(0., xmin=0, xmax=1, linestyle=':', alpha=0.5, color='grey')
axs[3].set_ylim(0., 360.)
#axs[1].set_title('Corrected', fontsize=fontSizeX, loc='left')
axs[3].set_xlim(datetimeShip[0], datetimeShip[-1])
