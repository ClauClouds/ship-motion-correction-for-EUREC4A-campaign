...
def generate_preprocess(): # can take arguments
    ...
    def preprocess(ds):
        ...
        Var2keep = ['rr', 'ta', 'pa', 'rh', 'wspeed', 'wdir', 'LWP', 'n_levels', 'no_chirp_seq', 'range_offsets', 'dz_chirp',\
         'dv_chirp', 'sample_tms', 'Doppler_velocity', 'ku', 'ze', 'sw','tb','sk', 'spec', 'sens_limit', 'mean_noise', 'vm_corrected_smoothed']
...
for indDay in range(NdaysEurec4a):
    ...
    sk = day_data['sk'].values
    ku = day_data['ku'].values

    spec = day_data['spec'].values
    n_levels = day_data['n_levels'].values
    n_chirp_seq = day_data['no_chirp_seq'].values
    range_offset = day_data['range_offsets'].values
    dz_chirp = day_data['dz_chirp'].values
    dv_chirp = day_data['dv_chirp'].values
    sample_tms = day_data['sample_tms'].values
    kurtosis = day_data['ku'].values
    sens_limit = day_data['sens_limit'].values
    mean_noise = day_data['mean_noise'].values
    doppler_velocity =  day_data['Doppler_velocity'].values
    ...
    WbandData = xr.Dataset(
        data_vars={
            ...
            'liquid_water_path':(('time',), lwp,{'long_name': 'liquid water path', 'units':'g m-2'}),
            'brightness_temperature':(('time',), tb,{'long_name': 'brightness temperature at 89 GHz', 'units':'K'}),
            ...
            'kurtosis':(('time','height'), ku, {'long_name': 'kurtosis', 'units':''}),

            'doppler_velocity': (('time','height','doppler'), day_data['Doppler_velocity'].values,  {'long_name': 'doppler velocity', 'units':'m s-1'}), # leave units intentionally blank, to be defined in the encoding
            'doppler_spectrum':(('time','height','doppler'), spec, {'long_name': 'Doppler spectrum', 'units':'mm6 m-3'}),
            'n_levels':(('scalar'), n_levels, {'long_name': 'Number of range levels', 'units':''}),
            'n_chirp_seq':(('scalar'), n_chirp_seq, {'long_name': 'Number of chirp sequences', 'units':''}),
            'range_offset':(('chirp'), range_offset, {'long_name': 'Index indicating the range gate where the next chirp starts (initialized at 1)', 'units':''}),
            'dz_chirp':(('chirp'), dz_chirp, {'long_name': 'range resolution for chirp sequence', 'units':'m'}),
            'sens_limit':(('time','height'), sens_limit, {'long_name': 'linear sensitivity limit for chirp sequence', 'units':'mm6 mm-3'}),
            'mean_noise':(('time','height'), mean_noise, {'long_name': 'bin Doppler spectrum mean noise power', 'units':'mm6 mm-3'}),
            'dv_chirp':(('chirp'), dv_chirp, {'long_name': 'Doppler velocity resolution for chirp sequence', 'units':'m s-1'}),
            "sample_tms": (('time',), sample_tms, {'long_name': 'milliseconds of sample', 'units':'mu s', "standard_name": "milliseconds of sample", "comment":"To get the correct time the variable sample_tms must be added: time = time + sample_tms"}),
        ...
        coords={
            ...
            'doppler': (('doppler',), np.arange(512), { "axis": "D", "standard_name": "doppler dimensions"}),
            'scalar': (('scalar',), np.arange(1), {"long_name":'variable for scalar dimension'}),
            'chirp': (('chirp',), np.arange(3), {"long_name":'chirp dimension'}),
        ...
        attrs={'CREATED_BY'     : 'Claudia Acquistapace',
                        ...
                        'INSTRUMENT_MODEL' : '94 GHz (W-band) radar',
    ...
    # storing ncdf data
    WbandData.to_netcdf(path_out+dateReverse+'_wband_radar_msm_eurec4a_full.nc', encoding={"radar_reflectivity":{"zlib":True, "complevel":9},\
                                                                                             ...
                                                                                             "doppler_spectrum": {"zlib": True, "complevel":9}, \
                                                                                             "doppler_velocity": {"zlib": True, "complevel":9}, \
                                                                                             "lat": {"dtype": "f4"} , \
                                                                                             "lon": {"dtype": "f4"}, \
